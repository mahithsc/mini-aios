...
def bash(cmd: str, timeout: float = 30):
    ...
    # Cron jobs execute on scheduler worker threads; avoid writing their output
    # into the interactive terminal session.
    if out and threading.current_thread() is threading.main_thread():
...
def cron(action: str, name: str = None, description: str = None,
    ...
    if action == "create":
        ...
        try:
            cid = get_cron_manager().create_cron(name, description, instructions, schedule)
            return f"Cron created: {cid} ({name})"
        except ValueError as e:
            return f"error: invalid schedule -- {e}"

    elif action == "list":
        return get_cron_manager().list_crons()

    elif action == "edit":
        if not cron_id:
            return "error: edit requires cron_id"
        try:
            return get_cron_manager().edit_cron(
        ...
        return get_cron_manager().delete_cron(cron_id)
