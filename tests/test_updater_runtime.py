...
def test_updater_api_auth_drain_ready_and_resume(
    ...
    for directory in (state, skills, workspace):
        directory.mkdir()
    database = state / "aios.db"
    with sqlite3.connect(database) as connection:
        connection.execute("CREATE TABLE test (id INTEGER PRIMARY KEY)")

    control = RuntimeControl(state / "update-drain.json")
    monkeypatch.setattr(runtime_control, "_runtime_control", control)
    monkeypatch.setattr(
        updater,
        "get_runtime_paths",
        lambda: SimpleNamespace(state=state, skills=skills, workspace=workspace),
    )
    monkeypatch.setattr(
        updater,
        "get_db_connection",
