...
_FORBIDDEN_FILENAMES = {".netrc", ".npmrc", ".pypirc"}
_FORBIDDEN_SUFFIXES = {
    ".db",
    ".key",
    ".p12",
...
def find_deployment_root(start: str | Path) -> Path | None:
    ...
    A coding agent may be launched in a component directory such as
    ``app/backend`` while the artifact manifest intentionally lives at ``app``.
    Deployment tools must package that manifest root, not whichever nested
    directory the shell happens to be using.
    """
    ...
    for path in artifact_file_paths(root):
        ...
        with path.open("rb") as handle:
            while chunk := handle.read(1024 * 1024):
                ...
                digest.update(chunk)
                overlap = scan_window[-_SECRET_SCAN_OVERLAP_BYTES:]
...
def artifact_file_paths(app_dir: str | Path) -> list[Path]:
    ...
    for current, directories, filenames in os.walk(root, followlinks=False):
        ...
        for directory in sorted(directories):
            if directory in _IGNORED_PARTS:
                continue
