...
_IDEMPOTENCY_KEY = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._:-]{0,191}$")
...
class CloudDeployClient:
    ...
    def create_app(self, name: str) -> dict[str, Any]:
        """Reserve an app identity before a coding agent creates its manifest."""
    ...
    def deploy(
        ...
        with tempfile.TemporaryDirectory(prefix="aios-artifact-") as directory:
            ...
            if getattr(archive.manifest, component) is None:
                raise CloudDeployError(
                    f"Deployment component is not declared in the manifest: {component}"
                )
    ...
    def deploy_pipeline(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix="aios-artifact-") as directory:
            ...
            requested = _normalize_components(
    ...
    def enqueue_pipeline(
        ...
        *,
        ...
        return self._request_json(
            ...
            headers={"Idempotency-Key": key},
            json={"artifact_id": artifact_id, "components": requested},
        )

    def get_deployment_pipeline(self, pipeline_id: str) -> dict[str, Any]:
        return self._request_json("GET", f"/v1/deployment-pipelines/{pipeline_id}")
    ...
    def upload_app_media(
        ...
        *,
        ...
        if not path.is_file() or not path.is_relative_to(root):
            raise CloudDeployError(
                "Media file must be inside the current app workspace"
            )
        ...
        if (
            ...
            raise CloudDeployError(
                "Cloud returned an invalid media upload registration"
            )
        with self._client_factory() as client:
            response = client.put(
                upload_url,
...
def _require_idempotency_key(value: str | None) -> str:
    if not isinstance(value, str) or not _IDEMPOTENCY_KEY.fullmatch(value):
        raise CloudDeployError(
            "A stable idempotency_key is required for deployment pipelines"
        )
    return value
...
def _normalize_components(
    components: list[DeploymentComponent],
    *,
    declared: list[DeploymentComponent] | None = None,
) -> list[DeploymentComponent]:
    if not components:
        raise CloudDeployError("A deployment pipeline needs at least one component")
    invalid = sorted(set(components).difference(_COMPONENT_ORDER))
    if invalid:
        raise CloudDeployError("Unknown deployment component(s): " + ", ".join(invalid))
    component_set = set(components)
    requested = [
        component for component in _COMPONENT_ORDER if component in component_set
    ]
    if declared is not None:
        undeclared = [component for component in requested if component not in declared]
        if undeclared:
            raise CloudDeployError(
                "Deployment component(s) are not declared in the manifest: "
                + ", ".join(undeclared)
            )
    return requested
