...
class CronManager:
    ...
    def _schedule_job(self, cron_id: str, name: str, instructions: str, schedule: str):
        """Register or replace a job on the live scheduler."""
        # APScheduler allows adding jobs before start(); this is needed during
        # startup when we load persisted crons from the database.
    ...
    def start(self):
        ...
        for cron_id, name, instructions, schedule in rows:
            self._schedule_job(cron_id, name, instructions, schedule)
    ...
    def edit_cron(self, cron_id: str, **fields) -> str:
        """Update fields on an existing cron and reschedule if needed."""
        ...
        with self._get_conn() as conn:
            ...
            row = conn.execute(
                "SELECT id, name, instructions, schedule FROM crons WHERE id = ?",
                (cron_id,),
            ).fetchone()

        if row:
            self._schedule_job(*row)
